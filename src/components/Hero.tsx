...
const Hero = () => {
  ...
  const backgroundRef = useParallaxEffect(0.3);
  const magneticButtonRef = useMagneticEffect();

  useEffect(() => {
    const tl = gsap.timeline();

    // Enhanced hero entrance with professional timing
    ...
    .fromTo(buttonsRef.current?.children,
      ...
      { opacity: 1, y: 0, scale: 1, rotationY: 0, duration: 1, stagger: 0.2, ease: "back.out(1.4)" },
      "-=0.8"
    );

    // Professional floating elements animation
    if (floatingElementsRef.current) {
      const floatingElements = floatingElementsRef.current.querySelectorAll('.floating-element');
      
      // Entrance animation
      gsap.fromTo(floatingElements,
        { opacity: 0, scale: 0, rotation: -180, filter: "blur(5px)" },
        { 
          opacity: 1, 
          scale: 1, 
          rotation: 0,
    ...
    <section ref={heroRef} className="min-h-screen bg-gradient-to-br from-green-50 via-white to-emerald-50 flex items-center justify-center px-4 relative overflow-hidden">
      ...
      <div ref={floatingElementsRef} className="absolute inset-0 pointer-events-none hidden md:block">
        <div className="floating-element absolute top-20 left-10">
          <div className="w-12 lg:w-16 h-12 lg:h-16 bg-gradient-to-br from-green-100/95 to-green-200/95 rounded-full flex items-center justify-center shadow-2xl backdrop-blur-md border border-green-200/60">
            <Heart className="w-6 lg:w-8 h-6 lg:h-8 text-green-600" />
          </div>
        </div>
        
        <div className="floating-element absolute top-32 right-16">
          <div className="w-10 lg:w-12 h-10 lg:h-12 bg-gradient-to-br from-orange-100/95 to-orange-200/95 rounded-full flex items-center justify-center shadow-2xl backdrop-blur-md border border-orange-200/60">
            <Zap className="w-5 lg:w-6 h-5 lg:h-6 text-orange-600" />
          </div>
        </div>
        
        <div className="floating-element absolute bottom-32 left-20">
          <div className="w-12 lg:w-14 h-12 lg:h-14 bg-gradient-to-br from-blue-100/95 to-blue-200/95 rounded-full flex items-center justify-center shadow-2xl backdrop-blur-md border border-blue-200/60">
            <Sparkles className="w-6 lg:w-7 h-6 lg:h-7 text-blue-600" />
          </div>
        </div>

        <div className="floating-element absolute top-1/3 right-32">
          <div className="w-8 lg:w-10 h-8 lg:h-10 bg-gradient-to-br from-purple-100/95 to-purple-200/95 rounded-full flex items-center justify-center shadow-2xl backdrop-blur-md border border-purple-200/60">
            <Star className="w-4 lg:w-5 h-4 lg:h-5 text-purple-600" />
          </div>
        </div>

        <div className="floating-element absolute bottom-1/4 right-20">
          <div className="w-10 lg:w-12 h-10 lg:h-12 bg-gradient-to-br from-emerald-100/95 to-emerald-200/95 rounded-full flex items-center justify-center shadow-2xl backdrop-blur-md border border-emerald-200/60">
            <Leaf className="w-5 lg:w-6 h-5 lg:h-6 text-emerald-600" />
          </div>
        </div>

        <div className="floating-element absolute top-3/4 left-40">
          <div className="w-12 lg:w-14 h-12 lg:h-14 bg-gradient-to-br from-red-100/95 to-red-200/95 rounded-full flex items-center justify-center shadow-2xl backdrop-blur-md border border-red-200/60">
      ...
      <div className="max-w-6xl mx-auto text-center relative z-10 px-4">
        <div>
          ...
          <div className="inline-flex items-center bg-gradient-to-r from-green-100/90 via-emerald-50/90 to-green-100/90 backdrop-blur-md text-green-800 px-4 md:px-8 py-3 md:py-4 rounded-full text-xs md:text-sm font-bold mb-6 md:mb-8 shadow-2xl border border-green-200/40 hover:shadow-green-200/50 transition-all duration-500">
            <Sparkles className="w-4 md:w-5 h-4 md:h-5 mr-2 md:mr-3" />
            <span className="text-shimmer bg-gradient-to-r from-green-600 via-emerald-600 to-green-600 bg-clip-text text-transparent bg-[length:200%_100%]">
          ...
          <h1 ref={titleRef} className="text-4xl md:text-6xl lg:text-8xl font-black text-gray-900 mb-6 md:mb-8 leading-tight">
            ...
            <span className="relative inline-block">
              <span className="text-shimmer text-transparent bg-clip-text bg-gradient-to-r from-green-400 via-emerald-500 to-green-600 bg-[length:200%_100%]">
                Livora
              </span>
            </span>
          </h1>
          ...
          <div ref={buttonsRef} className="flex flex-col sm:flex-row gap-4 md:gap-6 justify-center items-center mb-16 md:mb-20 px-4">
            <div ref={magneticButtonRef}>
              <Button 
                ...
                className="w-full sm:w-auto bg-gradient-to-r from-green-500 via-emerald-500 to-green-600 hover:from-green-600 hover:via-emerald-600 hover:to-green-700 text-white px-6 md:px-10 py-4 md:py-6 text-lg md:text-xl rounded-2xl shadow-2xl hover:shadow-green-400/30 transform transition-all duration-700 group border border-green-400/30 hover:scale-105"
              >
            ...
            <Button 
              variant="outline" 
              className="w-full sm:w-auto border-2 border-green-200/80 bg-white/80 backdrop-blur-md text-green-700 px-6 md:px-10 py-4 md:py-6 text-lg md:text-xl rounded-2xl hover:bg-green-50/90 hover:border-green-300 hover:shadow-xl hover:shadow-green-200/20 transition-all duration-700 group hover:scale-105"
            >
          ...
          <div className="flex flex-col sm:flex-row flex-wrap justify-center items-center gap-4 md:gap-8 text-sm md:text-lg text-gray-600 mb-8 md:mb-12 px-4">
            <div className="flex items-center group cursor-pointer">
              <div className="w-3 md:w-4 h-3 md:h-4 bg-gradient-to-r from-green-400 to-green-600 rounded-full mr-2 md:mr-3 shadow-sm group-hover:shadow-green-400/50 group-hover:scale-125 transition-all duration-300"></div>
              <span className="font-semibold group-hover:text-green-600 transition-colors duration-300">AI-Powered Nutrition</span>
            </div>
            <div className="flex items-center group cursor-pointer">
              <div className="w-3 md:w-4 h-3 md:h-4 bg-gradient-to-r from-blue-400 to-blue-600 rounded-full mr-2 md:mr-3 shadow-sm group-hover:shadow-blue-400/50 group-hover:scale-125 transition-all duration-300"></div>
              <span className="font-semibold group-hover:text-blue-600 transition-colors duration-300">Doctor Reviewed</span>
            </div>
            <div className="flex items-center group cursor-pointer">
              <div className="w-3 md:w-4 h-3 md:h-4 bg-gradient-to-r from-purple-400 to-purple-600 rounded-full mr-2 md:mr-3 shadow-sm group-hover:shadow-purple-400/50 group-hover:scale-125 transition-all duration-300"></div>
        ...
        <div className="group cursor-pointer" onClick={scrollToWaitlist}>
          <ArrowDown className="w-6 md:w-8 h-6 md:h-8 text-green-600 mx-auto group-hover:text-green-700 transition-all duration-500 group-hover:translate-y-2 group-hover:scale-110 animate-bounce" />
        </div>
      </div>
    </section>
  );
